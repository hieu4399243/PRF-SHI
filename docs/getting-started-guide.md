...
File sinh ra khi chạy: `app/data/appointments.json` (lịch hẹn), `app/data/audit_log.jsonl` (log đã ẩn PII).
...
./.venv/bin/python -m app.reminder_worker --watch   # chạy nền, quét mỗi 60 giây
```
Chưa có điện thoại thật? Token giả sẽ được ghi vào `app/data/outbox/push_outbox.jsonl` để bạn xem.
...
```
In bảng **Accuracy / Macro-F1** cho 2 phiên bản triage (v1 vs v2) và ghi `docs/eval/results.md`.
...
| `HTTP 429 "Quá nhiều yêu cầu"` | Gửi quá nhanh (>30 req/60s per IP). Chờ hoặc giãn cách request thử nghiệm (script test dùng `time.sleep(0.1)` giữa request). Admin routes cũng bị giới hạn — nếu test bulk thay từng lệnh `curl` bằng vòng lặp chậm. |
...
**Đạt mốc khi:** chạy file tập, đặt được 1 lịch giả vào `app/data/appointments.json` và câu
...
app điện thoại gửi kèm `session` trong body JSON. Xem hàm `resolve_sid()` trong `app/app.py`.
...
**Đạt mốc khi:** đặt lịch được từ điện thoại, lịch hiện trong `app/data/appointments.json`.
...
> Quy ước khi học: tạo file tập trong `hoc/` (vd `docs/hoc/triage_demo.py`) để **không đụng
...
# Chạy hằng ngày
PORT=5001 ./.venv/bin/python -m app.app             # backend
./.venv/bin/python -m app.reminder_worker --watch   # worker nhắc (tùy chọn)
