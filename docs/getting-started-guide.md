...
File sinh ra khi chạy: `app/appointments.json` (lịch hẹn), `app/audit_log.jsonl` (log đã ẩn PII).
...
```
Chưa có điện thoại thật? Token giả sẽ được ghi vào `app/outbox/push_outbox.jsonl` để bạn xem.
...
**Đạt mốc khi:** chạy file tập, đặt được 1 lịch giả vào `app/appointments.json` và câu
...
**Đạt mốc khi:** đặt lịch được từ điện thoại, lịch hiện trong `app/appointments.json`.
