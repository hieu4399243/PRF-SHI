...
đều thấy slot trống → may được cả 2 lịch cùng khung (lỗi race condition). **UNIQUE INDEX
`ux_appointments_doctor_slot`** chặn điều này ở tầng DB:
```sql
CREATE UNIQUE INDEX ux_appointments_doctor_slot 
  ON appointments (doctor_id, date, time) WHERE status = 'confirmed';
```
Lưu ý: **khoá theo (doctor_id, date, time)** — 2 bác sĩ khác nhau có thể đặt cùng khung giờ,
