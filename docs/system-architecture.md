...
Client (Expo app + web demo)  ──HTTP /api/* JSON──►  Backend (Flask)  ──►  Lưu trữ
       ...
       └────────── Expo Push notification ─────────────────┘
                    app/reminder_worker.py (nền) ── quét lịch ──► push
```
...
- **Lưu trữ** — `app/storage.py` chọn Postgres/Supabase (khi có `DATABASE_URL`) hoặc file JSON.

## 2. Thành phần backend

| Thành phần | File | Trách nhiệm |
|-----------|------|-------------|
| API gateway | `app/app.py` | Routes Flask, `resolve_sid()`, trang admin. Chạy `0.0.0.0:5001`. |
| Nhạc trưởng hội thoại | `app/chatbot.py` | State machine (`SESSIONS` in-memory), điều phối các khối. |
| AI triage | `app/triage.py` | Phân loại triệu chứng → dịch vụ (v1/v2), Q&A dịch vụ, chỗ cắm LLM. |
| An toàn | `app/safety.py` | Cấp cứu→115, ẩn PII, chặn chẩn đoán, handoff, audit log. |
...
1. `app/app.py` → `resolve_sid()` xác định session (app: body `session`; web: cookie).
...
> `app/data.py` là seed dự phòng. Guardrail (`safety_patterns`) → DB chỉ mở rộng, luôn fail-safe.
...
**Rate limiting:** Tất cả `/api/*` routes bị giới hạn ~30 request / 60 giây per IP (`app/app.py` → `_is_rate_limited()`).
